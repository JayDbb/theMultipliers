...
const Register = (e) => {
    ...
    if (localStorage.getItem("PlayerRegistrationData")) {
        ...
        if (!found) {
            newID = parseInt(playerData[playerData.length - 1].id) + 1 //get whatever is the last ID in dataset
    ...
    if (localStorage.getItem("Score")) {
        playerScoreData = JSON.parse(localStorage.getItem("Score"))
        for (data of playerScoreData) {
    ...
    playerScoreData ? localStorage.setItem("Score", JSON.stringify([...dataHolder, userScoreData])) : localStorage.setItem("Score", JSON.stringify([userScoreData]))
